/*
 * Hub REST API
 * JetBrains Teamware Connector
 *
 * OpenAPI spec version: 2024.2.38971
 *
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

package com.jetbrains.hub;

import com.google.gson.annotations.SerializedName;
import io.swagger.v3.oas.annotations.media.Schema;

import java.util.ArrayList;
import java.util.List;
import java.util.Objects;

/**
 * UntrustedredirecturisPage
 */


public class UntrustedredirecturisPage extends BasePage {
    @SerializedName("untrustedredirecturis")
    private List<UntrustedRedirectURI> untrustedredirecturis = null;

    public UntrustedredirecturisPage untrustedredirecturis(List<UntrustedRedirectURI> untrustedredirecturis) {
        this.untrustedredirecturis = untrustedredirecturis;
        return this;
    }

    public UntrustedredirecturisPage addUntrustedredirecturisItem(UntrustedRedirectURI untrustedredirecturisItem) {
        if (this.untrustedredirecturis == null) {
            this.untrustedredirecturis = new ArrayList<UntrustedRedirectURI>();
        }
        this.untrustedredirecturis.add(untrustedredirecturisItem);
        return this;
    }

    /**
     * Get untrustedredirecturis
     *
     * @return untrustedredirecturis
     **/
    @Schema(description = "")
    public List<UntrustedRedirectURI> getUntrustedredirecturis() {
        return untrustedredirecturis;
    }

    public void setUntrustedredirecturis(List<UntrustedRedirectURI> untrustedredirecturis) {
        this.untrustedredirecturis = untrustedredirecturis;
    }


    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        UntrustedredirecturisPage untrustedredirecturisPage = (UntrustedredirecturisPage) o;
        return Objects.equals(this.untrustedredirecturis, untrustedredirecturisPage.untrustedredirecturis) &&
                super.equals(o);
    }

    @Override
    public int hashCode() {
        return Objects.hash(untrustedredirecturis, super.hashCode());
    }


    @Override
    public String toString() {
        String sb = "class UntrustedredirecturisPage {\n" +
                "    " + toIndentedString(super.toString()) + "\n" +
                "    untrustedredirecturis: " + toIndentedString(untrustedredirecturis) + "\n" +
                "}";
        return sb;
    }

    /**
     * Convert the given object to string with each line indented by 4 spaces
     * (except the first line).
     */
    private String toIndentedString(java.lang.Object o) {
        if (o == null) {
            return "null";
        }
        return o.toString().replace("\n", "\n    ");
    }

}
